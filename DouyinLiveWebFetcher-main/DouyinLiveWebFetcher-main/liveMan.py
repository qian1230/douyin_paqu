...
class DouyinLiveWebFetcher:
    ...
    def __init__(self, live_id, csv_save_path: str = "", abogus_file='a_bogus.js'):
    ...
    def _init_csv(self):
        ...
        if not os.path.exists(self.csv_save_path):
            with open(self.csv_save_path, "w", encoding="utf-8-sig", newline="") as f:
                w = csv.writer(f)
                w.writerow(["发送时间", "用户ID", "用户昵称", "弹幕内容","当前在线人数"])
    def start(self):
    ...
    def get_a_bogus(self, url_params: dict):
        ...
        获取 a_bogus
        """
        self.abogus_file = r"D:\pycharm\Project\DouyinLiveWebFetcher-main\DouyinLiveWebFetcher-main\a_bogus.js"
    ...
    def _connectWebSocket(self):
        ...
        import os
        # main.py所在目录的sign.js
        script_path = os.path.join(os.path.dirname(__file__), "sign.js")
        signature = generateSignature(wss, script_file=script_path)
    ...
    def _parseChatMsg(self, payload):
        ...
        if not self.csv_save_path:
            print("⚠️ 无CSV保存路径，跳过写入")
            return

        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        current_online = self.online_viewer
        try:
            # buffering=1 行缓冲，写完立刻落地磁盘
            with open(self.csv_save_path, "a", encoding="utf-8-sig", newline="", buffering=1) as f:
                w = csv.writer(f)
                w.writerow([now_str, user_id, user_name, content,current_online])
                f.flush()  # 强制清空缓冲区，立刻写入硬盘
            print(f"✅ CSV写入成功 | {now_str} | {user_name} | {content} | 在线人数:{current_online}")
        except Exception as e:
            print(f"❌ CSV写入失败，错误详情：{str(e)}")
