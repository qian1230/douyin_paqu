...
class RecordingsPage(PageBase):
    ...
    async def apply_filter(self):
        ...
        # 安全地更新 recording_card_area
        try:
            self.recording_card_area.update()
        except (AssertionError, AttributeError):
            # recording_card_area 还没有被添加到页面，跳过更新
            pass
    ...
    async def add_record_cards(self):
        ...
        try:
            from ...core.recording.record_manager import RecordingManager as _RM  # avoid unused import issues
        except Exception:
            _RM = None
        try:
            rec_ids = [getattr(r, "rec_id", None) for r in self.app.record_manager.recordings]
        except Exception:
            rec_ids = []
        with open('/home/yjh/StreamCap-main/.cursor/debug.log', 'a') as f:
            f.write(json_module.dumps({
                "id": "log_add_record_cards_entry",
                "timestamp": time_module.time() * 1000,
                "location": "recordings_view.py:add_record_cards",
                "message": "add_record_cards entry",
                "data": {
                    "recordings_count": len(self.app.record_manager.recordings),
                    "rec_ids": rec_ids,
                    "existing_cards_count": len(getattr(self.app.record_card_manager, "cards_obj", {})),
                },
        ...
        # 安全地更新 loading_indicator，如果它还没有被添加到页面则跳过
        if self.loading_indicator:
            try:
                self.loading_indicator.visible = True
                self.loading_indicator.update()
            except (AssertionError, AttributeError):
                # loading_indicator 还没有被添加到页面，跳过更新
                pass

        container_controls = self.recording_card_area.content.controls
        existing_control_keys = {getattr(ctrl, "key", None) for ctrl in container_controls}
        ...
        for recording in self.app.record_manager.recordings:
            if recording.rec_id not in self.app.record_card_manager.cards_obj:
                ...
                # 避免重复 append 已存在的卡片
                if existing_card.key not in existing_control_keys:
                    container_controls.append(existing_card)
        ...
        if cards_to_create:
            ...
            for card, recording in results:
                container_controls.append(card)
