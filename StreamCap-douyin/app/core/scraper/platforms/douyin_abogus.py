...
来源：参考 https://github.com/JoeanAmier/TikTokDownloader （GPLv3），
并在 https://github.com/Evil0ctal/Douyin_TikTok_Download_API 中继续维护。
此处仅为适配当前项目的最小实现，用于生成 Douyin Web 接口所需的 a_bogus。

说明：
- 依赖 gmssl 库。若环境缺少 gmssl，会回退为返回空字符串，调用方应在为空时跳过 a_bogus。
- 只保留了生成 a_bogus 的核心逻辑，去掉了与项目无关的部分。
"""
...
except Exception as e:  # pragma: no cover - 环境缺失 gmssl 时的兜底
    sm3 = None
    func = None
    logger.warning("gmssl 未安装，a_bogus 将被禁用: %s", e)
...
class ABogus:
    ...
    @staticmethod
    def random_list(a: float | None = None, b=170, c=85, d=0, e=0, f=0, g=0) -> list:
        r = a or (random() * 10000)
    ...
    def sm3_to_array(cls, data: str | list) -> list[int]:
        if sm3 is None or func is None:
            raise RuntimeError("gmssl 未安装，无法生成 a_bogus")
...
def get_a_bogus(url_params: dict | str, method: str = "GET") -> str:
    """
    生成 a_bogus；若 gmssl 未安装或失败则返回空字符串。
    """
    if sm3 is None or func is None:
        return ""
    try:
        value = ABogus().get_value(url_params, method=method)
        return quote(value, safe="")
    except Exception as e:  # pragma: no cover - 外部依赖失败时兜底
        logger.warning("生成 a_bogus 失败，返回空字符串: %s", e, exc_info=True)
        return ""
