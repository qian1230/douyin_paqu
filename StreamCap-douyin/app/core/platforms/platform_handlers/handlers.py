...
@PlatformHandler.register(r"v\.douyin\.com/")
class DouyinHandler(PlatformHandler):
    ...
    async def get_stream_info(self, live_url: str) -> StreamData:
        """
        使用“子进程 + 20 秒超时”调用 streamget，避免单个房间解析卡死整个事件循环。
        """
        # 子进程脚本路径
        worker_path = os.path.join(os.path.dirname(__file__), "douyin_worker.py")
        ...
        if self.record_quality:
            cmd.extend(["--quality", self.record_quality])
        ...
        try:
            ...
            # 超时：杀掉子进程，返回一个 is_live=False 的占位结果
            try:
                proc.kill()
        ...
        # 如果 returncode 非 0，先抛出异常（保持原来的错误处理流程）
        if proc.returncode != 0:
            # douyin_worker 出错时会输出 JSON 到 stdout，也可能有错误信息在 stderr
            # 尝试解析 JSON 获取更详细的错误信息
            try:
                payload = json.loads(stdout_text)
                if not payload.get("ok"):
                    error_msg = payload.get("error", "Unknown error")
                    raise RuntimeError(f"Douyin worker failed with code {proc.returncode}: {error_msg}")
            except json.JSONDecodeError:
                # 如果无法解析 JSON，使用原始错误信息
                msg = stderr_text or stdout_text
                raise RuntimeError(f"Douyin worker failed with code {proc.returncode}: {msg}")

        # returncode 为 0，尝试解析 JSON
        try:
            payload = json.loads(stdout_text)
        except json.JSONDecodeError as e:
            raise RuntimeError(f"Invalid JSON from Douyin worker: {e}")

        # 检查 payload 中的 ok 字段
        if not payload.get("ok"):
            error_msg = payload.get("error", "Unknown error")
            # 当 douyin_worker 返回错误时，抛出异常让上层捕获并处理
            raise RuntimeError(f"Douyin worker error: {error_msg}")

        data = payload.get("data") or {}

        return StreamData(
            platform=data.get("platform") or "抖音",
