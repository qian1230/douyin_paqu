...
from streamget import StreamData, DouyinLiveStream
...
class PlatformHandler(abc.ABC):
    ...
    @classmethod
    def register(cls: type[T], *patterns: str):
        """
        Register a platform handler class with one or more URL patterns.
        """
        def decorator(handler_cls: type["PlatformHandler"]) -> type["PlatformHandler"]:
            with cls._lock:
                for pattern in patterns:
                    cls._registry[pattern] = handler_cls
            return handler_cls
